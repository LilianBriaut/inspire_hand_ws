...
class ModbusHandler:
    def __init__(self, port, baudrate=115200, id=1):
        # Initialiser le ModbusSerialClient avec le port série et le baudrate
        self.client = ModbusSerialClient(method="rtu", port=port, baudrate=baudrate, timeout=1)
        try:
            if not self.client.connect():
                raise ConnectionException(f"Impossible de se connecter au dispositif : {port}, baudrate : {baudrate}")
            print(f"Connexion réussie : port série : {port}, baudrate : {baudrate}, ID : {id}")
...
class MainWindow(QMainWindow):
    def __init__(self, port, baudrate=9600):
        ...
        if self.device_id is not None:
            ...
            print("Aucun dispositif en ligne trouvé")

    def find_online_devices(self, port):
        for baudrate, rate_value in baud_rates.items():
            for device_id in range(100):  # Plage d'ID supposée : 0 à 99
                modbus = ModbusHandler(port, rate_value, device_id)
                res = modbus.read_register(1000, 1)  # Tentative de lecture du registre 1000
                if res is not None:
                    print(f"Dispositif en ligne trouvé : ID = {device_id}, baudrate = {rate_value}")
                    modbus.close()
...
if __name__ == '__main__':
    ...
    window = MainWindow(port='/dev/ttyUSB1', baudrate=115200)  # Remplacer par le nom du port série réel
    sys.exit(app.exec_())
